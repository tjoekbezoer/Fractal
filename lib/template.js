...
(function( exports ) {
	...
	View.prototype = {
		...
		// should set it to `undefined`. If there is a default value defined for this property,
		// trying to set it to `undefined` will result in the property being set to its default.
		updateState: function( state, value ) {
			...
			if( state ) {
				...
				// Clone the current state, or the defaults. Or create a new object if both are
				// undefined.
				this.state = this._cloneState(this.state || this.defaultState) || {};
				// Merge the new state properties. If a state value is `undefined` and there is
				// a default for it, the default value will be used instead of `undefined`.
				Object.keys(state).forEach(function( key ) {
					var newValue    = state[key];
					this.state[key] = newValue === undefined && this.defaultState ?
				...
				// Only set defaults.
				this.state = this._cloneState(this.defaultState);
			}
