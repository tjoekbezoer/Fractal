...
(function( exports ) {
	var _supportsTemplate   = 'content' in document.createElement('template')
	  , _reservedProperties = ['childViews', 'path', 'name'];
	
	function _validate( props ) {
		_reservedProperties.forEach(function( propName ) {
			if( propName in props ) throw new Error('Reserved property: '+propName);
		});
	}
	...
	View.prototype = {
		...
		_findTemplateReference: function( template ) {
			...
			if( path ) {
				var ref = Fractal.getReference(path);
