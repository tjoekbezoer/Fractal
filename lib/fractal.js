...
(function( exports ) {
	...
	function _define( Source, Parent, props, path, name ) {
		var autoParent = false;
		if( !Parent || Parent.constructor == Object ) {
			props      = Parent;
			Parent     = Source.prototype.defaultView;
			autoParent = true;
		}
		
		props || (props = {});
		if( !props.defaultView && autoParent ) {
			props.defaultView = Source.prototype.defaultView;
		}
		props.childViews = {};
	...
	function _virtual( Source, parentName, props ) {
		var Parent = Source.prototype.childViews[parentName] ||
		             Source.define(parentName);
	...
	var View = function( state, parent ) {
		this.parent = parent;
	...
	View.extend = function extend( protoProps, staticProps ) {
		return _.inherit(this, {
			prototype: protoProps || {},
			static:    staticProps
		});
	};
	View.define = function define( name, Parent, protoProps ) {
		...
		return this.prototype.childViews[name] = _define(
			this,
			Parent,
			protoProps,
			this.prototype.path,
	...
	View.virtual = function virtual( parentName, name, protoProps ) {
		this.prototype.childViews || (this.prototype.childViews = {});
		this.prototype.childViews[name] = _virtual(
			this,
			parentName,
			protoProps
		);
	...
	View.prototype = {
		...
		// Only use this manually when defining root templates (`Fractal.define`).
		// When defining child templates use the static `View.define`.
		// 
		// This method is also used to automatically create an empty child view class
		// when trying to `add` an undefined child view.
		define: function( name, Parent, protoProps ) {
			return this.childViews[name] = _define(
				this.constructor,
				Parent,
				protoProps,
				this.path,
		...
		_createChild: function( viewName, state ) {
			...
			return new viewClass(state, this);
		},
		_getChildView: function( viewName ) {
			// If there is a local definition for this template, return that.
			var viewClass = this.childViews[viewName];
			if( viewClass ) return viewClass;
			
			// Not local; maybe it's a reference template?
			var template = Fractal.getLocalTemplate(this, viewName)
			  , ref      = template && template.getAttribute('data-ref');
			if( !template || !ref ) return undefined;
			
			// Try to find the view this template's ref-ing to.
			var crumbs = ref.split('.')
			  , view   = Fractal.constructor;
			crumbs.forEach(function( crumb ) {
				view = view.prototype.childViews[crumb];
				return !view;
			});
		...
		// Methods for handling <template> elements
		// ----------------------------------------
		// 
		_initTemplate: function() {
			if( !this.template ) {
				var local    = Fractal.getLocalTemplate(this.parent, this.name)
	...
	// The `Fractal` singleton
	// ========================
	// 
	var Fractal = new (View.extend({
		_tree:        undefined,
		// For actual template views, this object is created in `_createChildView`.
		childViews:  {},
		
		constructor: function() {},
		// These methods are not available on `Fractal`.
		out:         undefined,
		updateState: undefined,
		
		setDefaultView: function( View ) {
			this.constructor.prototype.defaultView = View;
		},
		
		virtual: function( parentName, name, protoProps ) {
			this.childViews[name] = _virtual(
				this.constructor,
				parentName,
				protoProps
			);
		...
		add: function() {
			if( !this._tree ) Fractal._processTemplates();
		...
		getLocalTemplate: function( view, name ) {
			var fragmentNodes = view !== this ?
			                    view.template.fragmentNodes :
			                    [document.body];
			
			var match;
			fragmentNodes.some(function find( el ) {
				// This code allows templates to be nested directly, instead of requiring
				// a wrapping DOM element. This code is disabled because it might cause unexpected
				// behavior:
				//   _ parent
				//     |_ info
				//   _info
				// This will mingle the nested `info` and the root `info` in the same 'namespace',
				// confusing Fractal.
				// 
				// if(
				// 	el.nodeName && el.nodeName.toLowerCase() == 'template' &&
