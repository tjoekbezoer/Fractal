...
(function( exports ) {
	...
	View.extend = function( props, statics ) {
		...
		_extend(Child.mixin(props.mixins) || Child.prototype, props);
	...
	View.mixin = function( mixinName ) {
		if( mixinName instanceof Array ) {
			...
			var mixin = Fractal.getMixin(mixinName)
			  , def   = typeof mixin == 'function' ? mixin(this) : mixin;
			
			if( !def ) return;
			// We can stop if this mixin is already applied to this view.
			var mixins = this.prototype._mixins || (this.prototype._mixins = {});
			if( mixinName in mixins ) return;
			mixins[mixinName] = true;
			
			// First mix in other required mixins.
